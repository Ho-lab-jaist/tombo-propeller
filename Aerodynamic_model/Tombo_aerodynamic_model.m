...
xN                  = (29-ma2/2)*10^-3;         %position of start point of Nodus [m]
XN                  = (29+ma2/2)*10^-3;         %position of end point of Nodus [m]
...
%Percent of propotions
% Df                  = 0.94*10^-3;                %diameter of f_fiber [mm]
Df                  = Df*10^(-3);
df                  = 0.37*10^-3;                %diameter of D_fiber [mm]
ND                  = 6;                         %Number of D_fiber;
Nd                  = 0;                         %Number of d_fiber;
Vf                  = (Df^2*pi*ND/4+df^2*Nd/4)/A_RN; %Volume fraction of fiber [100%]
...
theta1              = PoPP_2_Xcp(1:4,7);             %Angle of attach;
...
Td2 = Tr2;
Dgd2 = Dgr2;
% F_ctl = 0; %ommit the centrifugal force
CoS = [x_cos y_cos z_cos]';
CoM_design = [x_com y_com z_com]';
% eul = [ep sig dp];
% rotmZYX = eul2rotm([ep sig dp])
...
%         cos(sig)*cos(ep).*Dgd2_f1(u)./cos(dp)...
%         .*CD_fun(u).*(u.^2);
